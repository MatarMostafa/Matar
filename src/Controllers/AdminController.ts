...
export const getSperrungen = (req: Request, res: Response) => {
  const sperrListe: Record<string, string[]> = {}; // { kundeEmail: [mitarbeiterName, ...] }

  mitarbeiter.forEach((m) => {
    m.gesperrtFuer?.forEach((kunde) => {
      if (!sperrListe[kunde]) sperrListe[kunde] = [];
...
export const entsperreMitarbeiter = (req: Request, res: Response) => {
  const { mitarbeiterId, kundeEmail } = req.body;

  const m = mitarbeiter.find((m) => m.id === mitarbeiterId);
  if (!m) return res.status(404).json({ message: "Mitarbeiter nicht gefunden" });
...
import { Request, Response } from "express";
import { auftraege } from "../data/auftraege";
import dayjs from "dayjs"; // Stelle sicher, dass dayjs installiert ist

export const getDurchschnittMitarbeiter = (req: Request, res: Response) => {
  const wochenStatistik: Record<string, { total: number; count: number }> = {};
  const monatsStatistik: Record<string, { total: number; count: number }> = {};

  auftraege.forEach((auftrag) => {
    const datum = dayjs(auftrag.datum);
...
import { exportAuftraegeAlsCSV } from "../Utils/exportToCSV";
import { auftraege } from "../data/auftraege";
import { Request, Response } from "express";

export const exportiereAuftraege = (req: Request, res: Response) => {
  const csv = exportAuftraegeAlsCSV(auftraege);
...
export const einsatzstundenProMitarbeiter = (req: Request, res: Response) => {
  const statistik: { [mitarbeiterId: string]: number } = {};

  for (const auftrag of auftraege) {
    const dauerInStunden = parseFloat(auftrag.dauer);
    if (!auftrag.eingeteilteMitarbeiter) continue;
  ...
  const ergebnis = Object.entries(statistik).map(([id, stunden]) => {
    const mitarbeiterInfo = mitarbeiter.find(m => m.id === id);
    return {
      id,
...
export const durchschnittswerte = (req: Request, res: Response) => {
...
export const kundenMonatsstatistik = (req: Request, res: Response) => {
  const statistikMap: Record<string, {
    auftraege: number;
    gesamtDauer: number;
    bewertungen: number[];
  }> = {};
  ...
  for (const auftrag of auftraege) {
    const datum = parseISO(auftrag.datum);

    if (!isThisMonth(datum)) continue;
...
export const gesamtauswertungMonat = (req: Request, res: Response) => {
  let auftragCount = 0;
  let einsatzCount = 0;
  let gesamtStunden = 0;
  const einsaetzeProMitarbeiter: Record<string, number> = {};
  const tage: Set<string> = new Set();
  ...
  const beliebtester = Object.entries(einsaetzeProMitarbeiter).sort(
    (a, b) => b[1] - a[1]
  )[0];
...
export const durchschnittlicherEinsatz = (req: Request, res: Response) => {
  let auftragCount = 0;
...
export const geleisteteStunden = (req: Request, res: Response) => {
  const stundenProMitarbeiter: Record<string, number> = {};

  for (const auftrag of auftraege) {
    const dauer = parseFloat(auftrag.dauer); // Dauer z. B. "4"

    if (!auftrag.eingeteilteMitarbeiter || isNaN(dauer)) continue;
...
export const auftraegeProKunde = (req: Request, res: Response) => {
  const anzahlProKunde: Record<string, number> = {};
...
export const durchschnittlicheMitarbeiteranzahl = (req: Request, res: Response) => {
  if (auftraege.length === 0) {
    return res.json({ durchschnitt: 0 });
  }

  const summe = auftraege.reduce((acc, auftrag) => {
    return acc + (auftrag.anzahlMitarbeiter || 0);
  }, 0);

  const durchschnitt = summe / auftraege.length;

  res.json({ durchschnitt: durchschnitt.toFixed(2) });
};
export const gesamteEinsatzzeit = (req: Request, res: Response) => {
  ...
  auftraege.forEach((auftrag) => {
    ...
    const istImMonat = 
      auftragsDatum.getMonth() === aktuellerMonat &&
      auftragsDatum.getFullYear() === aktuellesJahr;

    if (!istImMonat || !auftrag.eingeteilteMitarbeiter) return;

    const bestaetigte = auftrag.eingeteilteMitarbeiter.filter(
      (m) => m.status === "bestätigt"
    );
...
export const einsatzzeitProMitarbeiter = (req: Request, res: Response) => {
  const jetzt = new Date();
  const aktuellerMonat = jetzt.getMonth();
  const aktuellesJahr = jetzt.getFullYear();
  ...
  auftraege.forEach((auftrag) => {
    ...
    auftrag.eingeteilteMitarbeiter.forEach((eintrag) => {
      if (eintrag.status !== "bestätigt") return;
...
export const durchschnittswerte = (req: Request, res: Response) => {
  ...
  auftraege.forEach((auftrag) => {
    ...
    if (bestaetigte.length === 0) return;
  ...
  const durchschnittStunden =
    zaehlerAuftraege > 0 ? summeStunden / zaehlerAuftraege : 0;
  const durchschnittMitarbeiter =
    zaehlerAuftraege > 0 ? summeMitarbeiter / zaehlerAuftraege : 0;
...
export const exportiereAuftraege = (req: Request, res: Response) => {
  ...
  const filePath = path.join("exports", dateiname);
...
export const kundenStatistik = (req: Request, res: Response) => {
  const statistik: { [kunde: string]: number } = {};
...
export const einsatzStatistik = (req: Request, res: Response) => {
  const statistik: { [monat: string]: { auftraege: number; stunden: number } } = {};
...
export const exportiereAuftraege = (req: Request, res: Response) => {
  try {
    ...
    const parser = new Parser({ fields: felder });
...
export const geleisteteStundenProMitarbeiter = (req: Request, res: Response) => {
  const ergebnis: Record<string, { name: string; stunden: number }> = {};

  auftraege.forEach((auftrag) => {
    const dauer = Number(auftrag.dauer) || 0;

    if (!auftrag.eingeteilteMitarbeiter) return;

    auftrag.eingeteilteMitarbeiter.forEach((zugewiesen) => {
      if (zugewiesen.status === "bestätigt") {
        const mid = zugewiesen.mitarbeiterId;
        const mitarbeiterInfo = mitarbeiter.find((m) => m.id === mid);
...
import { Request, Response } from "express";
import { auftraege } from "../data/auftraege";
import { Parser } from "json2csv"; // externes Paket!

export const exportiereAuftraegeAlsCSV = (req: Request, res: Response) => {
  try {
    const fields = [
      "id",
      "taetigkeit",
...
export const getBewertungen = (req: Request, res: Response) => {
...
export const exportiereAuftraegeAlsCSV = (_req: Request, res: Response) => {
  try {
    const fields = [
      "id",
      "taetigkeit",
...
export const stundenProMitarbeiter = (_req: Request, res: Response) => {
  const ergebnisse: { mitarbeiterId: string; name: string; stunden: number }[] = [];

  for (const m of mitarbeiter) {
    let stunden = 0;

    for (const a of auftraege) {
      if (!a.eingeteilteMitarbeiter) continue;
...
export const durchschnittswerte = (_req: Request, res: Response) => {
...
export const exportiereAuftraege = (_req: Request, res: Response) => {
  const data = JSON.stringify(auftraege, null, 2); // schön formatiert
...
export const getStundenProMitarbeiter = (_req: Request, res: Response) => {
  const stundenMap: Record<string, number> = {}; // mitarbeiterId → Stunden
  ...
  const ergebnis = mitarbeiter.map((m) => ({
    id: m.id,
    name: m.name,
...
export const getDurchschnittswerte = (_req: Request, res: Response) => {
...
export const getStundenUebersicht = (_req: Request, res: Response) => {
  const uebersicht: Record<string, number> = {};

  for (const auftrag of auftraege) {
    const dauer = parseFloat(auftrag.dauer);

    if (isNaN(dauer)) continue;
...
export const getDurchschnittswerte = (_req: Request, res: Response) => {
  ...
  for (const auftrag of auftraege) {
    ...
    if (bestaetigt.length === 0) continue;
...
export const geleisteteStundenProMitarbeiter = (req: Request, res: Response) => {
  ...
  for (const auftrag of auftraege) {
    if (!auftrag.eingeteilteMitarbeiter || !auftrag.dauer) continue;
  ...
  const ergebnis = Object.entries(stundenMap).map(([id, stunden]) => {
    const mitarbeiterName = mitarbeiter.find(m => m.id === id)?.name || "Unbekannt";
    return {
      mitarbeiterId: id,
...
export const setzeKategorie = (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  const mitarbeiterEintrag = mitarbeiter.find(m => m.id === id);
...
export const automatischEinplanen = (req: Request, res: Response) => {
  const offeneAuftraege = auftraege.filter(a => 
    a.status === "offen" && !a.eingeteilteMitarbeiter
  );
  ...
  for (const auftrag of offeneAuftraege) {
    ...
    let verfuegbar = mitarbeiter.filter(m => m.aktiv);
...
export const getStundenProMitarbeiter = (req: Request, res: Response) => {
  const stundenMap: Record<string, { name: string; anzahlAuftraege: number; gesamtStunden: number }> = {};

  for (const a of auftraege) {
    if (!a.eingeteilteMitarbeiter || !a.dauer) continue;

    for (const e of a.eingeteilteMitarbeiter) {
      if (e.status !== "bestätigt") continue;
...
export const getRotsperrungen = (req: Request, res: Response) => {
  const rotBewertungen = mitarbeiter
...
// CSV-Export aller Aufträge
export const exportiereAuftraegeCSV = async (req: Request, res: Response) => {
  try {
    const filePath = path.join(__dirname, "../../exports/auftraege.csv");
    ...
    await csvWriter.writeRecords(auftraege);
...
export const stundenProMitarbeiter = (req: Request, res: Response) => {
...
// src/Controllers/AdminController.ts
import { Request, Response } from "express";
import { sperrungAufheben } from "../Utils/sperrListe";
import { mitarbeiter } from "../data/mitarbeiter";

// Admin hebt die Sperrung für einen Mitarbeiter bei einem Kunden auf
export const sperreAufheben = (req: Request, res: Response) => {
  ...
  sperrungAufheben(mitarbeiterId, kundeId);
...
function farbeZuPunkt(farbe: string): number {
  if (farbe === "grün") return 3;
  if (farbe === "gelb") return 2;
  if (farbe === "rot") return 1;
  return 0;
}

// ─────────────────────────────
// Admin sieht Übersicht aller Mitarbeiter-Bewertungen
export const getBewertungsUebersicht = (req: Request, res: Response) => {
  const daten = mitarbeiter.map((m) => {
    ...
    const durchschnitt =
      gesamt > 0
        ? (
            bewertungen.reduce(
              (sum, b) => sum + farbeZuPunkt(b.farbe),
              0
            ) / gesamt
          ).toFixed(2)
        : "—";
...
export const exportiereAuftraegeCsv = (req: Request, res: Response) => {
  ...
  const csv = jsonToCsv(daten);
  const dateiname = `auftraege_${Date.now()}.csv`;
  const pfad = path.join(__dirname, "../../exports", dateiname);

  writeFileSync(pfad, csv, "utf-8");
...
export const bearbeiteQualifikationen = (req: Request, res: Response) => {
...
export const getArbeitsstunden = (req: Request, res: Response) => {
  const result: Record<string, { name: string; stunden: number }> = {};
